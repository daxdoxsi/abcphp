...
class Lang
{
    ...
    public static function readDB():void {
        ...
        $sql = 'SELECT * FROM devtools_lang WHERE 1';
    ...
    public static function t(string $text):string
    {
        ...
        if ( !isset(static::$dictionary[$text]) ) {
            ...
            $sqlA = 'SELECT * FROM devtools_lang WHERE text_code = :text';
            $resA = static::$db->query($sqlA, [':text' => $text]);

            # If text code does not exists then insert a new record into the database
            if (count($resA[0]) == 0) {

                $sql = "INSERT INTO `devtools_lang` (`lang_id`, `text_code`, `es`, `en`, `pt`, `it`, `fr`) VALUES (NULL, :text, NULL, NULL, NULL, NULL, NULL);";
